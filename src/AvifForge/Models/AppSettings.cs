...
public partial class AppSettings : ObservableObject
{
    ...
    /// <summary>同时转换的文件数（每文件一个 avifenc 进程）。默认 1：单文件串行，占用可预期。</summary>
    [ObservableProperty]
    ...
    /// <summary>设置结构版本：保存时统一写 3；低于 3 的旧文件在 Load 时触发一次性迁移。1.0.1→2（10-bit），1.0.2→3（输出目录/重命名/元数据/并发默认）。</summary>
    public int SchemaVersion { get; set; } = 1;

    public int MaxParallelism => Math.Clamp(Environment.ProcessorCount, 4, 16);
